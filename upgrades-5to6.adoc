...
[source,bash]
----
$ DJANGO_SETTINGS_MODULE=settings.local python manage.py migrate --noinput
$ DJANGO_SETTINGS_MODULE=settings.local python manage.py compilemessages
$ DJANGO_SETTINGS_MODULE=settings.local python manage.py collectstatic --noinput
----
...
If you are not using the `async tasks` service, we strongly recommend you to do. Update your `local.py` file to have `CELERY_ENABLED=True`, the corresponding credentials, and create the systemd unit as referred in link:setup-production.html#expose-taiga-async[these instructions] to set up the service.

Restart the taiga-async service.
...
$ cd ~/taiga-events
$ git checkout master
$ git rebase && git reset --hard origin/master
----
