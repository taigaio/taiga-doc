...
Due to the nature of frontend application, you should know that this service will be used
through the domain/public-ip. This is because the frontend application will run in your browser
...
Issue the following commands in order to install the required dependencies for Taiga(front-end,backend,events):
...
**taiga-back** uses postgresql (>= 9.4) as database:
sudo apt-get install -y postgresql-9.5 postgresql-contrib-9.5
sudo apt-get install -y postgresql-doc-9.5 postgresql-server-dev-9.5

To run **taiga-back**, python (3.5) must be installed along with a few third-party
libraries.
First install python and virtualenvwrapper:
[source,bash]
----
...
variables and functions. **This is a mandatory step before continuing the installation**

Create the taiga user give it root permissions
[source,bash]
----
sudo adduser taiga
...
~~~~~~~~~~~~~~~~
Setup the initial user and database in postgresql:
...
Download the code from github and install its dependencies:
...
This creates a the administrator. The login credentials are **admin** with password **123123**.

**OPTIONAL**
If you want some example data loaded into Taiga, execute the following command which populates the database
...
To finish the setup of **taiga-back**, you must create the intial configuration
for proper static/media file resolution and optionally for email sending support:

.Copy-paste this into ~/taiga-back/settings/local.py
[source,python]
----
from .common import *
...
To make sure everything is working, you can run the backend in development mode by executing the following command:
