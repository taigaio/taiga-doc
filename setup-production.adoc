...
The default behavior in Taiga is to do all tasks syncronously, but some of then
may be completelly asyncronous (for example webhooks or import/export). To do
this, you have to configure celery and install the celery service requirements.
...
For run celery with taiga you have to include in your local.py the lines:
...
You can configure other broker or results backends. You can se more about celery
broker/results backends configuration in the celery documentation web page:
http://docs.celeryproject.org/en/latest/index.html

Once you have configured celery on Taiga, you have to add celery to run in
circus. See link:#circus-and-gunicorn[Circus and gunicorn] section.
