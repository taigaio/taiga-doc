...
`taiga-front/dist/js/conf.example.json` to `taiga-front/dist/js/conf.json`
and editing it.

.Copy and edit initial configuration on ~/taiga-front/dist/js/conf.json
[source,json]
----
{
...
Now, having **taiga-front** downloaded and configured, the next step is to expose the code
...
Taiga allows adding functionality through contrib plugins. The instalation of each
