...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
This is a short system architecture description to help you understand the way Taiga is built and runs before you go further in the installation.
Taiga consists of 2 core (mandatory) modules:
- **taiga-back**
- **taiga-front**

**Taiga-back** is written in python and serves API endpoints for the frontend.
To run Taiga, the backend must be executed and monitored by **circus** and **gunicorn**.
Technically the backend reads the database(postgresql) and allows the frontend to read its content.
